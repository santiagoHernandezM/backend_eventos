...
export class EventoService {
  ...
  async crearEvento(evento: eventoDto) {
    ...
    if (eventosEncontrados.length > 0) {
      eventosEncontrados.map((eventoInstructor) => {
        eventoInstructor.eventos.map((eventoReportado) => {
          evento.eventos.map((eventosAReportar, indexEventoAReportar) => {
            let diaConnflicto = [];
            ...
            if (
              ...
              mensajeConflict.push({
                evento: { ...eventoReportado, mes: eventosAReportar.mes },
    ...
    //Verifico si hay eventos en conflicto y los saco para crear los que están bien
    if (mensajeConflict.length > 0) {
      console.log(mensajeConflict);

      const copia_evento = { ...evento };
      copia_evento.eventos = evento.eventos.filter(
        (evento, indexEvento) => !indexEventosConflicto.includes(indexEvento),
      );
      evento = { ...copia_evento };
    }

    let registrosEventoExistente = [];
    let eventosCrear = [];
    let respGestor: any;
    if (evento.eventos.length > 0) {
       ...
       */
      respGestor = await this.gestorTService.actualizarTiempos(evento);
        ...
        if (indexReporte == -1) {
          ...
          if (eventosCod.length > 0) {
            ...
            eventosCod.forEach((ev) => {
              const indexMes = repor.meses.findIndex(
                (mes) => mes.mes == ev.mes,
              );
              indexMes == -1
      ...
      registrosEventoExistente = await this.eventoModel.find({
    ...
    //Retorno los eventos en conflicto
    if (mensajeConflict.length > 0) {
